...
function updateBatchControls(job) {
  ...
  ui.autoOcrControl.dataset.tooltip = job?.status === "cancelling"
    ...
    : automaticOn
      ...
      : "Automatic OCR is off — switch on to process all pages";
...
async function runAllOcr() {
  ...
  try {
    ...
    if (["queued", "running", "cancelling"].includes(job.status)) {
      ...
      toast(`OCR queued for ${job.total_pages} page${job.total_pages === 1 ? "" : "s"}`);
    } else {
      state = await api("/api/state");
      renderState({ refreshPreview: false });
      toast(job.status === "completed" ? "All pages already have OCR results" : (job.error || "OCR batch could not start"), job.status === "completed" ? "info" : "error");
    }
  } catch (error) {
    ui.autoOcrToggle.checked = false;
...
async function pollOcrJob(jobId) {
  ...
  try {
    ...
    if (!["queued", "running", "cancelling"].includes(job.status)) {
      ...
      state = await api("/api/state");
      renderState({ refreshPreview: false });
      if (job.status === "completed") toast("OCR completed for all pages");
...
ui.autoOcrToggle.addEventListener("change", () => {
  if (ui.autoOcrToggle.checked) runAllOcr();
  else cancelAllOcr();
});
